...
# Written by Yiming Chen (chen1ming@cuc.edu.cn)
# Modified by Bin Xiao (leoxiaobin@gmail.com)
...
def make_heatmaps(image, heatmaps):
    ...
    num_joints, height, width = heatmaps[0].shape
...
def make_tagmaps(image, tagmaps):
    num_joints, height, width, n = tagmaps[0].shape
    image_resized = cv2.resize(image, (int(width), int(height)))
    ...
    for j in range(num_joints):
        ...
        tagmap = cv2.cvtColor(tagmap[:, :, 0], cv2.COLOR_RGB2BGR)
