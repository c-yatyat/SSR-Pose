...
import torchvision.ops as ops  # dcn
...
class BasicBlock(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_cbam=False, # nam
                 use_nam=False,
                 shape=32, # 135 in model_resnet.py from nam
                 no_spatial=True):
        ...
        if use_nam:
            self.nam = Att(out_channels, no_spatial=self.no_spatial, shape=shape)
        else:
            self.nam = None
        # ---
...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        if use_cbam:
            self.cbam = CBAM(out_channels * 4, 16)
        else:
            self.cbam = None
        
        if use_nam:
            self.nam = Att(out_channels * 4, no_spatial=self.no_spatial, shape=shape)
...
# todo: copy from DEKR >lib>models>conv_block.py
